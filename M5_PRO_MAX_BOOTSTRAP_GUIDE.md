...
## 🎁 [사전 준비: 대표실 디스코드 인증 정보 확보 (1분 컷)]

수석 PM `Hermes`가 대표님의 개인 비서로서 작동하고, 사내 건강 지표가 대표실 디스코드 채널로 다이렉트 직송되려면 아래 **딱 2가지 고유 보안 정보**가 필요합니다. 이는 대표님의 개인 권한 영역이므로 최초 1회 직접 복사해 두셔야 합니다.

1. **PM Hermes 구동용 봇 토큰 (Bot Token):**
   - [디스코드 개발자 포털](https://discord.com/developers/applications) 접속 ➡️ 애플리케이션 생성 ➡️ `Bot` 메뉴에서 **`Reset Token`**을 클릭해 생성된 토큰을 복사해 둡니다.
2. **비상 감사 보고용 웹훅 URL (Webhook URL):**
   - 대표실 디스코드 서버의 알림을 받으실 채널의 **[채널 편집(톱니바퀴)]** ➡️ **[연동(Integrations)]** ➡️ **[웹훅 만들기]**를 눌러 생성된 **`웹훅 URL 복사`**를 클릭해 둡니다.

*(이 두 값은 아래 1단계 터미널 시동 명령어 가동 시 최초 1회 입력 창이 뜰 때 그대로 복사-붙여넣기만 해주시면 에이전트가 `.env`에 알아서 안전하게 파싱 적재합니다.)*

---

## 👔 [대표님 박청룡님이 하실 유일한 초기 2단계]

새로 수령하신 M5 Pro Max에서 AI 에이전트의 완전 자율 비대화형 구동(백그라운드 자동 깃 푸시 등)을 영구적으로 확보하기 위해, 대표님께서는 최초 1회 아래의 **초간단 2단계**만 직접 승인해 주시면 모든 준비가 완결됩니다.

### 1단계: 1줄 시동 명령어로 PM 깨우기
터미널(Terminal) 앱을 열고 다음 **단 1줄의 마스터 커맨드**를 실행해 주십시오.
```bash
curl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash && hermes setup
```
*(이 명령어는 Nous Research의 최첨단 **Hermes Agent** 구동 엔진을 대표님의 맥북에 최초 영구 적재하고, 대표님의 디스코드 메신저 토큰을 안전하게 바인딩하여 PM Hermes를 영구 비서로 깨우는 초기화 단계입니다.)*

### 2단계: macOS 키체인 접근 제어 항상 허용 세팅 (최초 1회 필수)

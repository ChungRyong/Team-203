# [Team-203] 에이전트별 페르소나 및 백엔드 인프라 명세서 (v1.5)
**작성일:** 2026-05-26
**작성자:** 3실장
...
### ① 3진 아웃 징계 작동 메커니즘
1. **경고 누적 (`warning_count`):** `Blinky` 주임은 에이전트들의 린트 위반, 오류 미수정, 기술 사양/리소스 날조(Hallucination) 등을 모니터링하여 `POST /api/agents/{agent_name}/penalize`를 호출하고 경고를 1회씩 누적시킵니다.

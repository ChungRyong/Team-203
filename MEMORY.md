...
- [ ] **2순위: orchestrator.py 신설 (최우선 과제 - 대표님 결재 완료):**
  - [ ] 한 번에 한 에이전트만 VRAM에 올리는 직렬 큐 오케스트레이션 루프 구현.
  - [ ] **[이원화 설계 반영]** 단순 코드 리뷰 반려(CTO Reject) 시 경고 적립 없이 재수정 턴 환류 ➡️ 고의성 린트 무시 및 날조(Hallucination) 시에만 Blinky 징계 스택 적립.
  - [ ] 징계 시 `recommended_temperature = 0.0` 강제 잠금 및 `pardon` API를 통한 뇌/컨텍스트 세척 연동.

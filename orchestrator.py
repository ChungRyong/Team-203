...
class VirtualStudioOrchestrator:
    ...
    def log_audit(self, event_type, status, details=None, elapsed_ms=None):
        """
        Sends an operational log to the FastAPI /api/audit/log endpoint.
        """
        payload = {
        ...
        res = self.call_api("POST", "/audit/log", payload)
        if res and res.status_code == 201:
            self.log(f"🛡️ Audit log written: [{event_type}] -> {status}")
        else:
            self.log(f"⚠️ Failed to write audit log for [{event_type}]")
    ...
    def initialize_studio(self, title, description):
        ...
        self.log_audit(
            ...
            status=status_str,
            details={"action": "initialize_studio", "task_id": self.task_id, "room_id": self.room_id},
            elapsed_ms=elapsed
        )
        ...
        if res:
            if res.status_code == 200:
                ...
                if res_data.get("status") == "success":
                    ...
                    status_str = "WARNING"
                details.update(res_data)
                self.log(f"VRAM for '{model_name}' successfully requested to unload.")
            else:
                self.log(f"VRAM unload returned error status {res.status_code}.")
        else:
    ...
    def run_agent_turn(self, agent_name, base_prompt, model_name):
        ...
        if res and res.status_code == 200:
            ...
            warning_count = prompt_data.get("warning_count", 0)
            if is_penalized:
                self.log(f"⚠️ [WARNING] 에이전트 '{agent_name}'은 징계 상태입니다. '절대 팩트 모드' 접두사가 인젝션되며 온도가 0.0으로 강제 잠금됩니다.")
                self.log_audit(
                    event_type="BLINKY_PENALTY",
                    status="WARNING",
                    details={"agent_name": agent_name, "action": "enforce_penalty_lock", "warning_count": warning_count, "is_penalized": is_penalized}
                )
        else:
        ...
        if not res or res.status_code != 200:
            ...
            self.log_audit(
                event_type="SESSION_COMPRESS",
                status="FAILED",
                details={"agent_name": agent_name, "error": "failed_to_load_context", "room_id": self.room_id}
            )
            return None
    ...
    def execute_cto_review_flow(self, file_or_dir_path):
        ...
        if not enabled:
            ...
            elapsed = int((time.perf_counter() - start_time) * 1000)
            self.log_audit(
                event_type="CTO_REVIEW",
                status="SUCCESS",
                details={"status": "bypassed_by_config", "file_path": file_or_dir_path},
                elapsed_ms=elapsed
            )
        ...
        try:
            ...
            if sub.returncode == 0:
                ...
                self.log_audit(
                    event_type="CTO_REVIEW",
                    status="SUCCESS",
                    details={"file_path": file_or_dir_path, "stdout": sub.stdout[:500] if sub.stdout else ""},
                    elapsed_ms=elapsed
                )
                return "PASSED"
            else:
                ...
                self.log_audit(
                    ...
                    details={
                        ...
                        "stdout": sub.stdout[:500] if sub.stdout else "",
                        "is_pardon_applied": is_pardon_applied
                    },
                    elapsed_ms=elapsed
                )
                return "REJECTED"
            ...
            self.log_audit(
                event_type="CTO_REVIEW",
                status="WARNING",
                details={"error_detail": str(e), "file_path": file_or_dir_path},
                elapsed_ms=elapsed
            )
    ...
    def finalize_studio(self, milestone_summary):
        """
        Step 7: Close meeting room, generate Markdown audit diary, and broadcast milestone summary to Discord.
        """
        ...
        if summary_res and summary_res.status_code == 200:
            ...
            vram_health = summary_data.get("vram_health", 100.0)
        ...
        # 4. Generate Obsidian-compatible audit diary file on physical disk
        import datetime
        today_str = datetime.datetime.now().strftime("%Y%m%d")
        
        base_dir = os.path.dirname(os.path.abspath(__file__))
        audit_dir = os.path.join(base_dir, "workspace", "audit")
        os.makedirs(audit_dir, exist_ok=True)
        
        diary_filepath = os.path.join(audit_dir, f"audit_diary_{today_str}.md")
        
        # Determine Blinky / Hermes advice
        advice = "모든 시스템의 VRAM 관리 및 정적 CTO 검수가 완벽하게 작동하고 있습니다. 지속적인 안정화 방치가 가능합니다."
        if health_score < 90.0:
            advice = "사내 기강 수준(디시플린 레벨) 또는 백업 안정성이 다소 하락했습니다. Dev-Agent의 에셋 검출 및 warnings 누적을 조치하기 위해 사면 API 발송 준비를 권고합니다."
        if health_score < 80.0:
            advice = "🚨 [비상 경보] 종합 오피스 건강성이 80% 미만으로 훼손되었습니다! OOM 감지 또는 API 429 한계선 돌파 조짐이 보입니다. 대표님의 직속 감사 개입을 권고합니다."
            
        diary_content = (
            f"# 📊 가상 사옥 정기 경영 감사 일지 (audit_diary_{today_str})\n\n"
            f"## 📈 사내 건강성 지수 (Office Health Index): **[{health_score}%]**\n"
        ...
        try:
            ...
            self.log(f"📝 감사 일지 생성 완료: '{diary_filepath}'")
        except Exception as e:
            self.log(f"🚨 Failed to write audit diary file: {e}")
            
        # 5. Post Professional final comprehensive PM milestone report to Discord
        discord_content = (
        ...
        # If health score is under 80%, trigger Red Emergency Embed (color code: red 16711680)
        # Else trigger Professional blue embed (color code: blue 3447003)
        if health_score < 80.0:
            embed_color = 16711680 # Red
            embed_title = f"🚨 [비상 경보] {self.task_id} 오피스 붕괴 위험 감지!"
        else:
            embed_color = 3447003 # Nice Blue
            embed_title = f"🏆 [마일스톤 완료] {self.task_id} 성공 배포 보고"
